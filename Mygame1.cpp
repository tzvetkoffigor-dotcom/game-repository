...
// Ñòðóêòóðà äëÿ õðàíåíèÿ ðåêîðäîâ
struct HighScore {
    string playerName;
    int score;
};
        ...
        while (getline(file, line)) {
            ...
            if (commaPos != string::npos) {
                ...
                try {
                    ...
                    // Èãíîðèðóåì íåêîððåêòíûå ñòðîêè
                }
            }
        }
        file.close();
...
// Èãðîâàÿ ëîãèêà - óçêèå ïëàòôîðìû è áîëüøèå ðàññòîÿíèÿ
void PlayGame() {
    struct Platform {
        float x, y;
        float width = 45;  // ?? ÓÇÊÈÅ ÏËÀÒÔÎÐÌÛ (áûëî 70)
        float height = 12; // ?? ÒÎÍÜØÅ (áûëî 15)
        Color color = GREEN;
    };
    ...
    // ?? ÓÂÅËÈ×ÅÍÍÛÅ ÐÀÑÑÒÎßÍÈß ÌÅÆÄÓ ÏËÀÒÔÎÐÌÀÌÈ
    const float MAX_JUMP_HEIGHT = 150.0f;   // Óâåëè÷èë âûñîòó ïðûæêà
    const float PLATFORM_SPACING = 110.0f;  // ?? ÁÎËÜØÅ ÐÀÑÑÒÎßÍÈÅ (áûëî 90)
    ...
    for (int i = 0; i < VISIBLE_PLATFORMS; i++) {
        ...
        // ?? ÀÄÀÏÒÈÐÎÂÀÍÍÛÅ ÏÎÇÈÖÈÈ ÄËß ÓÇÊÈÕ ÏËÀÒÔÎÐÌ
        if (i % 3 == 0) {
            platform.x = 50; // Ëåâàÿ ñòîðîíà
        }
        else if (i % 3 == 1) {
            platform.x = screenWidth / 2 - platform.width / 2; // Öåíòð
        }
        else {
            platform.x = screenWidth - 50 - platform.width; // Ïðàâàÿ ñòîðîíà
        }
    ...
    // ?? ÑÁÀËÀÍÑÈÐÎÂÀÍÍÀß ÔÈÇÈÊÀ ÄËß ÁÎËÜØÈÕ ÐÀÑÑÒÎßÍÈÉ
    const float MAX_JUMP_FORCE = -14.0f;    // Óâåëè÷èë ñèëó ïðûæêà
    const float MIN_JUMP_FORCE = -7.0f;
    const float GRAVITY = 0.5f;
    const float MAX_FALL_SPEED = 18.0f;     // Óâåëè÷èë ñêîðîñòü ïàäåíèÿ
    const float MOVE_SPEED = 6.0f;          // Óâåëè÷èë ñêîðîñòü äâèæåíèÿ

    // Ïåðåìåííûå äëÿ ïðûæêà
    ...
    while (!WindowShouldClose() && !gameOver) {
        ...
        // Ãåíåðàöèÿ íîâûõ ïëàòôîðì ñ óçêèìè íàñòðîéêàìè
        if (platforms.back().y > -100) {
            ...
            // ?? ÀÄÀÏÒÈÐÎÂÀÍÍÛÅ ÏÎÇÈÖÈÈ ÄËß ÓÇÊÈÕ ÏËÀÒÔÎÐÌ
            switch (patternIndex) {
            case 0: newPlatform.x = 40; break;                       // Ëåâî
            case 1: newPlatform.x = screenWidth / 2 - 22; break;      // Öåíòð
            case 2: newPlatform.x = screenWidth - 40 - newPlatform.width; break; // Ïðàâî
            case 3: newPlatform.x = 80; break;                       // Ëåâî-öåíòð
            case 4: newPlatform.x = screenWidth - 120 - newPlatform.width; break; // Ïðàâî-öåíòð
            case 5: newPlatform.x = screenWidth / 2 - 22; break;      // Öåíòð
            }
        ...
        // ?? ÎÒÐÈÑÎÂÊÀ ÓÇÊÈÕ ÏËÀÒÔÎÐÌ Ñ ßÐÊÈÌÈ ÊÎÍÒÓÐÀÌÈ
        for (auto& platform : platforms) {
            DrawRectangle(platform.x, platform.y, platform.width, platform.height, platform.color);
            // ßÐÊÈÅ ÊÎÍÒÓÐÛ ÄËß ËÓ×ØÅÉ ÂÈÄÈÌÎÑÒÈ ÓÇÊÈÕ ÏËÀÒÔÎÐÌ
        ...
        // ?? ÎÒËÀÄÎ×ÍÀß ÈÍÔÎÐÌÀÖÈß Î ÐÀÇÌÅÐÀÕ
